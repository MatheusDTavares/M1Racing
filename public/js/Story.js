...
function conteudo() {
    ...
    if(imagem == 1) {
        textoM.innerHTML = `<div class="text"><h4>Como surgiu meu interesse pelo tema?</h4><p>Meu interesse por Fórmula 1 surgiu ainda na infância, entre 2011 e 2012. 
          ...
          Naquela época, meus pais estavam construindo nossa casa, porque ainda morávamos na casa dos meus avós.</p></div>
          <div style="display: flex; align-items: center; gap: 40px; flex: 1;"><img src="./assets/imgs/WhatsApp Image 2026-05-18 at 23.26.54.jpeg"></div><br>`
    } else if (imagem == 2) {
        textoM.innerHTML = `<div class="text"><h4>Como surgiu meu interesse pelo tema?</h4><p>
        ...
        textoM.innerHTML = `<div class="text"><h4>Como esse tema está presente no meu dia a dia?</h4><p>Hoje em dia, 
        ...
        Por isso escolhi esse tema para meu projeto individual: ele une algo que sempre fez parte da minha história com algo que quero seguir profissionalmente.</p></div>`
    } else if (imagem == 6) {
         textoM.innerHTML = `<div class="text"><h4>Valores representados no projeto</h4><p>Meu projeto representa vários valores importantes para mim:
...
Paixão por tecnologia, principalmente pela análise de dados e desempenho presente na Fórmula 1.</p></div>`
    } else if (imagem == 7) {
         textoM.innerHTML = `<div class="text"><h4>Momentos marcantes que reforçaram essa paixão</h4><p>Mesmo sem nunca ter corrido de kart quando era criança, 
         ...
         chamada por nós de “Neguinha”. Ele chegou a competir com ela utilizando o número 43,
          algo que marcou muito minha infância.</p></div>`
    } else if (imagem == 8) {
         textoM.innerHTML = `<div class="text"><h4>Maior dificuldade no projeto</h4><p></p></div><br>`
    } else if (imagem == 9) {
         textoM.innerHTML = `<div class="text"><h4>Maior superação</h4><p>Minha maior superação foi conseguir unir minhas duas maiores paixões: tecnologia e automobilismo. 
         Mais do que apenas criar um site, 
         consegui desenvolver algo que representa minha história, 
         minhas memórias e os momentos que vivi com minha família.</p></div>`
    } else if (imagem == 10) {
         textoM.innerHTML = `<div class="text"><h4>Agradecimentos</h4><p>Grande parte dessa paixão surgiu por causa do meu pai. 
         ...
         então esse projeto também representa uma homenagem a tudo que vivi ao lado dele.</p></div>`
    }
